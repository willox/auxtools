...
signatures! {
	...
	send_maps => "55 8B EC 6A FF 68 ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? 50 81 EC ?? ?? ?? ?? A1 ?? ?? ?? ?? 33 C5 89 45 F0 53 56 57 50 8D 45 F4 ?? ?? ?? ?? ?? ?? A0 ?? ?? ?? ?? 04 01 75 05 E8 ?? ?? ?? ?? E8"
}
...
signatures! {
	...
	send_maps => "55 89 E5 57 56 53 81 EC ?? ?? ?? ?? 80 3D ?? ?? ?? ?? ?? 0F 84 ?? ?? ?? ??"
}
...
byond_ffi_fn! { auxtools_init(_input) {
	...
	if get_init_level() == InitLevel::Full {
		...
		unsafe {
			...
			raw_types::funcs::send_maps_byond = send_maps;
		}
