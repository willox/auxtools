...
signatures! {
	...
	execute_instruction => "0F B7 48 ?? 8B 78 ?? 8B F1 8B 14 ?? 81 FA 59 01 00 00 0F 87 ?? ?? ?? ??",
	invalid_instruction => "52 68 ?? ?? ?? ?? E8 ?? ?? ?? ?? 83 C4 08 A1 ?? ?? ?? ?? 66 FF 40 ?? E9 ?? ?? ?? ??"
}
...
byond_ffi_fn! { auxtools_init(_input) {
	...
	if get_init_level() == RequiredInitLevel::Full {
		...
		unsafe {
			raw_types::funcs::execute_instruction_byond = execute_instruction;
			raw_types::funcs::INVALID_INSTRUCTION = invalid_instruction;
