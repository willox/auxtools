...
pub use dm_impl::{hook, init, runtime_handler, shutdown};
...
signatures! {
	...
	is_type => "55 8B EC 8B 4D 08 0F B6 C1 48 83 F8 53 77 22 0F B6 ?? ?? ?? ?? ?? FF 24 ?? ?? ?? ?? ?? FF 75 14 FF 75 10 FF 75 0C 51",
	text_to_path => "55 8B EC 83 EC 08 56 57 8B 7D 08 57 E8 ?? ?? ?? ?? 83 C4 04 8B 30 A1 ?? ?? ?? ?? 89 45 FC A1 ?? ?? ?? ?? 89 45 F8 81 FF"
}
...
byond_ffi_fn! { auxtools_init(_input) {
	...
	if get_init_level() == InitLevel::Full {
		...
		unsafe {
			...
			raw_types::funcs::is_type_byond = is_type;
			raw_types::funcs::text_to_path_byond = text_to_path;
		}
