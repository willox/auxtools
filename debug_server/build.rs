...
fn main() {
	...
	match env::var("CARGO_CFG_TARGET_FAMILY").unwrap().as_str() {
		"unix" => { build.file("src/execute_instruction_hook.unix.S"); }
		"windows" => match env::var("CARGO_CFG_TARGET_ENV").unwrap().as_str() {
			"gnu" => { build.file("src/execute_instruction_hook.windows.S"); }
			"msvc" => { build.file("src/execute_instruction_hook.windows.asm"); }
